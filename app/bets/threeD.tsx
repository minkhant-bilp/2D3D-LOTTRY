...
export default function ThreeDDetailScreen() {
    ...
    const handleQuickPick = (type: string) => {
        ...
        if (type === 'DIRECT') {
            if (inputVal.length !== 3) {
                Alert.alert('သတိပေးချက်', 'ဒဲ့ အတွက် ဂဏန်း (၃) လုံး အတိအကျ ထည့်ပါ။');
                return;
            }
            ...
            if (inputVal.length !== 3) {
                Alert.alert('သတိပေးချက်', 'R (အလှည့်) အတွက် ဂဏန်း (၃) လုံး အတိအကျ ထည့်ပါ။');
                return;
            }
            const d = inputVal.split('');
            const perms = new Set([
            ...
            numbersToAdd = ['000', '111', '222', '333', '444', '555', '666', '777', '888', '999'];
        }
        else if (type === 'THWAT') {
            if (inputVal.length !== 2) {
                Alert.alert('သတိပေးချက်', 'သွပ်ရန်အတွက် ဂဏန်း (၂) လုံး အတိအကျ ထည့်ပါ။ (ဥပမာ - 12)');
                return;
            }
            for (let i = 0; i <= 9; i++) {
                numbersToAdd.push(inputVal[0] + i + inputVal[1]);
            }
        }
    ...
    const handlePasteSubmit = () => {
        ...
        const matches = pasteText.match(/\b\d{3}\b/g);
        ...
        if (matches && matches.length > 0) {
            ...
            Alert.alert('အမှား', '(၃) လုံး ဂဏန်းများ ရှာမတွေ့ပါ။ ဖော်မတ် မှန်ကန်မှုရှိမရှိ စစ်ဆေးပါ။ (ဥပမာ - 123 456)');
        }
    };
                ...
                <View style={styles.titleRow}>
                    ...
                    <View style={styles.pillOuter}>
                        <View style={styles.pillInner}>
                            <Text style={styles.pillText}>3D ဈေးကွက်</Text>
                        </View>
                    </View>
                </View>
                        ...
                        <View style={styles.fastEntryOuter}>
                            <View style={styles.fastEntryInner}>
                                ...
                                <View style={styles.fastActionGrid}>
                                    ...
                                    <TouchableOpacity style={styles.fastActionBtn} onPress={() => handleQuickPick('THWAT')}>
                                        <Text style={styles.fastActionText}>သွပ်</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        </View>
                                ...
                                {betRows.map((row: any) => (
                                    <View key={row.id} style={styles.rowCardOuter}>
                                        <View style={styles.rowCardInner}>
                                            <View style={styles.rowCardHeader}>
                                                <View>
                                                    ...
                                                    <Text style={styles.potentialWinValue}>
                                                        {row.amount ? `x ${(Number(row.amount) * 500).toLocaleString()}` : '—'}
                                                    </Text>
                                                </View>
                                                <Pressable
                                            ...
                                            <Text style={styles.inputLabel}>ဂဏန်း (0-999)</Text>
